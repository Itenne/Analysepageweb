...
- `ui.py` and `exporters.py`: Tkinter desktop interface and CSV, Excel, and HTML reporting.
...
4. Select a result for DNS, status, indicator, and error details. Export CSV, Excel, or HTML.
...
A 403 alone remains `HTTP_403`. `ACCESS_DENIED` requires an access-denial HTTP status and corroborating configurable blocking signals (two signatures, or a signature plus proxy/SASE header). `IP_RESTRICTION` is prioritized when configured IP-specific text is observed. `AUTHENTICATION_REQUIRED` identifies configured sign-in or authentication-page signals, as well as HTTP 401/407 responses, and never submits credentials. Add languages or vendor wording in `config/signatures.json` without code changes.

> `ACCESS_DENIED`, `AUTHENTICATION_REQUIRED`, and `IP_RESTRICTION` are indications based on observed response data and are not definitive proof of the cause of a block.
