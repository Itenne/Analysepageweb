...
class ValidatorUI(ttk.Frame):
    ...
    def _poll(self):
        try:
            while True:
                ...
                elif kind=="result": self.results.append(value); self.tree.insert("","end",values=(value.url,"OK" if value.classification=="ACCESS_ALLOWED" else "AUTH" if value.classification=="AUTHENTICATION_REQUIRED" else "BLOCKED" if value.classification in {"ACCESS_DENIED","IP_RESTRICTION"} else "ERROR",value.http_status or "",value.final_url,value.public_ip,value.duration_ms or "",value.classification,value.confidence)); self.progress.step(1)
    ...
    def export_excel(self):
        path=filedialog.asksaveasfilename(defaultextension=".xlsx",filetypes=[("Excel workbook","*.xlsx")]); path and export_excel(self.results,path,self.public_ip)
